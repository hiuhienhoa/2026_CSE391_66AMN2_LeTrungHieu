...
Khi trang được tải, JavaScript ghi dòng "Hello from JavaScript!" vào Console của trình duyệt
---
...
> - JavaScript: xử lý logic và tương tác (sự kiện, thay đổi nội dung trang).
...
1 + 2 * 3; hiện ra 7
"Hello " + "world"; hiện ra 'Hello world'
```
...
Xin chào, mình là Hiếu, năm nay mình 20 tuổi.
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
thiếu ';' chạy bình thường
thiếu '+' thì sẽ báo lỗi như dòng dưới
Uncaught SyntaxError: missing ) after argument list (at main.js:7:13)
...
không cần dấu ';' vẫn chạy được
hoạt động như các ngôn ngữ lập trình (tính toán, in xâu, ..)
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
thiếu dấu '+' khi nối chuỗi 
em đã tìm ra và sửa lỗi nhờ chat GPT
---
...
cấu trúc nếu thì là cấu trúc rẽ nhánh
if(điều kiện)
thực hiện khi điều kiện đúng
else
thực hiện khi điều kiện sai
có thể có nhiều điều kiện ứng với nhiều công việc thì dùng thêm else if
ví dụ thực tế 
if (mưa to) //điều kiện 1
nghỉ học //thực hiện khi điều kiện 1 đúng
else if(mưa nhỏ) //điều kiện khác
đi học mang ô //thực hiện với điều kiện 2 đứng
else
đi học //thực hiện khi không thỏa mãn điều kiện nào ở trên
//có thể có nhiều hơn 1 else if hoặc không có cái nào
...
>   - Tên hàm: tên dùng để gọi và phân biệt hàm.
>   - Tham số (parameters): dữ liệu đầu vào truyền vào hàm.
>   - Thân hàm (body): các câu lệnh thực hiện chức năng của hàm.
>   - Giá trị trả về (return): .kết quả mà hàm trả lại sau khi thực hiện.
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
sử dụng lại hàm khi cần làm công việc tương tự,dễ bảo trì,dễ đọc,ngắn gọn.
---
...
đôi khi nhầm kiểu trong return và biến của hàm
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)?
hỏi bạn lỗi sai của mình, tra google 
---
...
> Câu hỏi:
> - `document.getElementById("status")` đang làm gì? tìm thẻ có id là status để tương tác
> - Sự kiện `"click"` xảy ra khi nào? khi ấn vào nút Chào
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang? dòng có id là status tức dòng 'Chưa có tương tác...' đổi 
thành 'Xin chào! Đây là nội dung được thay đổi bằng JavaScript.'
---
...
tăng kích thước chữ của toàn bộ trang web lên 20px.
...
input là khi người dùng nhập vào ô và ấn enter
click là khi người dùng ấn vào nút
...
Cả hai đoạn code đều gắn sự kiện "click" cho nút btnHello(Chào), khi người dùng nhấn nút thì sẽ hiển thị câu 'Xin chào! Đây là nội dung được thay đổi bằng JavaScript.'
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
JavaScript thuần:
Lấy phần tử: document.getElementById
Gắn sự kiện: addEventListener
jQuery:
Lấy phần tử: $
Gắn sự kiện: .on
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Đơn giản hóa thao tác DOM (tìm phần tử, thay đổi nội dung, style).
>   2. Xử lý sự kiện và hiệu ứng dễ dàng hơn so với JavaScript thuần.
